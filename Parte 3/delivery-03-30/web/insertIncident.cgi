...
date_time = str(date)+str(' ')+str(time)
...
try:
    ...
    # Making query
    sql = """ INSERT INTO incident (instant,id,description,severity) VALUES (%s,%s,%s,%s); """
    data = (date_time,id,description,severity)
    print('<p>{}</p>'.format(sql % data))
    cursor.execute(sql, data)
    connection.commit()

    print('<b><h1 style="text-align:center;",class="a">Incident inserted successfully!</h1><b>')
