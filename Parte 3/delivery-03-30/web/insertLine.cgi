...
id_l = form.getvalue('id_l')
impedance = form.getvalue('impedance')
...
try:
    ...
    connection.commit()
    sql = """ INSERT INTO line (id,impedance,pbbid,sbbid) VALUES (%s,%s,%s,%s); """
    data = (id_l,impedance,pbbid,sbbid)
    cursor.execute(sql, data)
    connection.commit()
    print('<b><h1 style="text-align:center;",class="a">Line inserted successfully!</h1><b>')
