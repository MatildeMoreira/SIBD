...
try:
    ...
    # Making query
    sql = """ INSERT INTO lineincident (instant,id,point) VALUES (%s,%s,%s); """
    data = (date_time,id,point)
    print('<p>{}</p>'.format(sql % data))
    cursor.execute(sql, data)
    connection.commit()

    print('<b><h1 style="text-align:center;",class="a">Line Incident inserted successfully!</h1><b>')
