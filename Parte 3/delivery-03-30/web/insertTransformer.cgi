...
id_t = form.getvalue('id_t')
volt_pv_t = form.getvalue('volt_pv_t')
volt_sv_t = form.getvalue('volt_sv_t')
gpslatitude = form.getvalue('gpslatitude')
gpslongitude = form.getvalue('gpslongitude')
pbbid_bus = form.getvalue('pbbid_bus')
sbbid_bus = form.getvalue('sbbid_bus')
...
try:
    ...
    # Making query
    sql = """ INSERT INTO element (id) VALUES (%s); """
    data = (id_t,)
    cursor.execute(sql, data)
    connection.commit()
    sql = """ INSERT INTO transformer (id,pv,sv,gpslat,gpslong,pbbid,sbbid) VALUES (%s,%s,%s,%s,%s,%s,%s); """
    data = (id_t,volt_pv_t,volt_sv_t,gpslatitude,gpslongitude,pbbid_bus,sbbid_bus)
    cursor.execute(sql, data)
    connection.commit()
    print('<b><h1 style="text-align:center;",class="a">Transformer inserted successfully!</h1><b>')
    print('<center><img src="insertion.png" style="width:500px;"></center>')
