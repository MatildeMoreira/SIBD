...
import psycopg2, cgi, time,datetime
...
try:
    ...
    sql = """ DELETE FROM lineincident WHERE instant = %s AND id = %s; """
    data = (instant,id)
    cursor.execute(sql, data)
    connection.commit()

    sql = """ DELETE FROM incident WHERE instant = %s AND id = %s; """
    data = (instant,id)
    cursor.execute(sql, data)
    connection.commit()


    print('<b><h1 style="text-align:center;",class="a">Line Incident removed successfully!</h1><b>')
